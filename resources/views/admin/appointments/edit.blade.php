...
@section('content')
<div class="max-w-2xl">
<form method="POST" action="{{ route('admin.appointments.update', $appointment) }}" class="bg-white rounded-xl shadow p-6 space-y-4">
    @csrf @method('PUT')
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div>
            ...
            <select name="service_id" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500">
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div>
            ...
            <select name="staff_id" class="w-full border border-gray-300 rounded-lg px-3 py-2">
                <option value="">Sin asignar</option>
                @foreach($staff as $s)
                    <option value="{{ $s->id }}" {{ old('staff_id',$appointment->staff_id)==$s->id?'selected':'' }}>{{ $s->name }}</option>
                @endforeach
            </select>
        </div>
    </div>
        ...
        <div>
            ...
            <select name="appointment_status" class="w-full border border-gray-300 rounded-lg px-3 py-2">
                @foreach(['pending'=>'Pendiente','confirmed'=>'Confirmada','completed'=>'Completada','cancelled'=>'Cancelada'] as $v=>$l)
                    <option value="{{ $v }}" {{ old('appointment_status',$appointment->appointment_status)===$v?'selected':'' }}>{{ $l }}</option>
                @endforeach
            </select>
        </div>
        <div>
            ...
            <select name="payment_status" class="w-full border border-gray-300 rounded-lg px-3 py-2">
                @foreach(['pending'=>'Pendiente','paid_in_clinic'=>'Pagado presencialmente','paid_online'=>'Pagado online','waived'=>'Exonerado'] as $v=>$l)
                    <option value="{{ $v }}" {{ old('payment_status',$appointment->payment_status)===$v?'selected':'' }}>{{ $l }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            ...
            <input type="number" name="total_price" value="{{ old('total_price',$appointment->total_price) }}" min="0" step="0.01"
                   class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Duración (min)</label>
            <input type="number" name="duration_minutes" value="{{ old('duration_minutes',$appointment->duration_minutes) }}" min="1"
                   class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500">
        </div>
    </div>
    ...
    <div>
        ...
        <textarea name="admin_notes" rows="2"
                  class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500">{{ old('admin_notes',$appointment->admin_notes) }}</textarea>
    </div>

    <div class="flex gap-3 pt-2">
        <button type="submit" class="bg-indigo-600 text-white px-6 py-2 rounded-lg hover:bg-indigo-700">Guardar Cambios</button>
        <a href="{{ route('admin.appointments.show', $appointment) }}" class="border border-gray-300 text-gray-600 px-6 py-2 rounded-lg hover:bg-gray-50">Cancelar</a>
    </div>
</form>
</div>
@endsection
